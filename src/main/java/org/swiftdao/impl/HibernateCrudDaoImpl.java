...
public class HibernateCrudDaoImpl<E extends Persistable> extends HibernateDaoSupport implements CrudDao<E> {
    ...
    protected Connection getConnection() throws SQLException {
        ...
        if (connection[0] == null) {
            throw new SQLException("Failed to get JDBC Connection from Hibernate Session");
        }
        return connection[0];
    }
            ...
            public Void doInHibernate(Session session) throws HibernateException {
                session.doWork(new Work() {
                    ...
                    public void execute(Connection conn) throws SQLException {
                        ...
                        buf.append("v").append(conn.getMetaData().getDatabaseMajorVersion()).append(".");
                        buf.append(conn.getMetaData().getDatabaseMinorVersion());
                    }
                });
                return null;
            }
    ...
    public List<E> findAll(final Class clazz) throws SwiftDaoException {
        ...
        List<E> entities = super.getHibernateTemplate().execute(new HibernateCallback<List<E>>() {
            ...
            public List<E> doInHibernate(Session session) throws HibernateException {
                ...
                return session.getCriteriaBuilder().createQuery(clazz).getOrderList();
            }
        });
        return entities;
    }
...
//		return this.getSession().createCriteria(clazz).list();
    ...
    public List<E> findByParamsPagination(Map<String, Object> paramMap,
        ...
        if (StringUtils.isNotBlank(orderParam)) {// 排序
    ...
    public long countByParams(Map<String, Object> paramMap,
                              ...
                              Map<String, Object> extraParams) throws SwiftDaoException {
        // paramMap可以为null，但是中不能存在空值
        if (paramMap != null && MapUtils.isContainsEmptyValue(paramMap)) {
...
//                }
            ...
            log.info("查询参数名称数：" + queryParamNames.length + " 和参数值数：" + queryParams.length);
...
//	}
    ...
    public void clearInCache() throws SwiftDaoException {
        this.getSessionFactory().getCurrentSession().evict(entityClass);
    }
